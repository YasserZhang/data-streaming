...
zSetEntriesDecodedStreamingDF.withColumn('customer', from_json('encodedCustomer', customerJSONSchema)) \
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stediEventsRawStreamingDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092")  \
...
joinedDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""
    email = customer
"""))
