...
spark = SparkSession.builder.appName("consumer-data").getOrCreate()
...
redisServerRawStreamingDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("subscribe","redis-server").option("startingOffsets", "earliest").load()
...
redisServerStreamingDF.withColumn("value", from_json("value", redisMessageSchema)).select(col("value.*")).createOrReplaceTempView("RedisSortedSet")
...
zSetEntriesDecodedStreamingDF.withColumn('customer', from_json('encodedCustomer', customerJSONSchema)).select(col('customer.*')).createOrReplaceTempView('CustomerRecords')
