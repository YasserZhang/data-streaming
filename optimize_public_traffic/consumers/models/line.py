...
FAUSTO_TABLE_CHANGELOG_TOPIC = "transformed.stations"
...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == FAUSTO_TABLE_CHANGELOG_TOPIC: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith(ARRIVAL_TOPIC_PREFIX): # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic() == TURNSTILE_SUMMARY_TOPIC: # Set the conditional to the KSQL Turnstile Summary Topic
